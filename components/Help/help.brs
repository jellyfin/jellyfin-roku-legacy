...
sub settingFocused()
    ...
    if selectedSetting.image <> invalid
        m.screenShot.uri = selectedSetting.image
        m.screenShot.visible = true
    else
        m.screenShot.visible = false
    end if
...
sub settingSelected()
    ...
    if selectedItem.children <> invalid and selectedItem.children.Count() > 0 ' Show sub menu
